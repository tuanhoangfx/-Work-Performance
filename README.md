...
**Note:** If you are updating an existing database, please make sure to run the `ALTER TABLE` commands to add the `email` column to the `profiles` table if it doesn't exist.
...
BEGIN
  ...
  VALUES (NEW.id, NEW.raw_user_meta_data->>'full_name', NEW.email, NEW.raw_user_meta_data->>'avatar_url', 'employee', NEW.created_at, NEW.last_sign_in_at)
...
-- handle_user_update for last_sign_in_at and email
